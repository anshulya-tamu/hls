...
NP=100 		# Run this many download processes by default. 
...
### earthdata account
if [ ! -f $HOME/earthdata_login/hls1/.netrc ]
then
	echo "$HOME/earthdata_login/hls1/.netrc file unavailable" >&2
	echo "Search the web for how to set up .netrc" >&2
	exit 1
else 
	if ! grep urs.earthdata.nasa.gov $HOME/earthdata_login/hls1/.netrc -q
	then
		echo "urs.earthdata.nasa.gov entry not found in $HOME/earthdata_login/hls1/.netrc" >&2
		exit 1
	fi
fi
