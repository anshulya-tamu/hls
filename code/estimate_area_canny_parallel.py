...
def process_clip_path(clip_path, merge_path, clf, res_idx, im_name, sat, bands):
  try:
    ...
    if sat == 'S30':
      max_x = extent_df.iloc[0,3]
      max_y = extent_df.iloc[0,4]
      max_file_name = os.path.join(extent_df.iloc[0,6],'S30/B05_stitched_raster.tif')
    else:
      max_x = extent_df.iloc[0,1]
      max_y = extent_df.iloc[0,2]
      max_file_name = os.path.join(extent_df.iloc[0,5],'L30/B05_stitched_raster.tif')
    ...
    i_frac = ice_mask[(ice_mask==1)&(jrc_data>0)].shape[0]/jrc_data[jrc_data>0].shape[0]
    ...
    if X[X==1].shape[0]==0:
      ...
      new_row = pd.DataFrame([{'date': merge_path.split('/')[-1],
                       ...
                       'threshold': 0,
                       'elbow': 0,
                       'ice_cover': i_frac*100,
                       'cloud_cover': 100}])
      return new_row

    w_frac = (raw_cloud[raw_cloud==1]).shape[0]/(X[X==1]).shape[0]
    print('Water ratio from Fmask is: {:.2f}'.format(w_frac))

    X[(cloud_mask==1)&(raw_cloud!=1)] = 2
    X[(water_mask==1)&(raw_cloud==0)] = 1
    X[ice_mask==1] = 3
    # X[(sm==0)&(X==1)] = 2
    X[(jrc_data==0)] = 0
  ...
  for i in range(len(clip_paths)):
    ...
    if os.path.isdir(os.path.join(clip_path,'S30')):
      if os.path.exists(os.path.join(str(res_idx),'classified_rasters',clip_path.split('/')[-1],'S30')):
        ...
        s30_clip_paths.append(clip_path)
        s30_merge_paths.append(merge_path)
    ...
    if os.path.isdir(os.path.join(clip_path,'L30')):
      if os.path.exists(os.path.join(str(res_idx),'classified_rasters',clip_path.split('/')[-1],'L30')):
        ...
        l30_clip_paths.append(clip_path)
        l30_merge_paths.append(merge_path)

  # Parallelize the loop
  with concurrent.futures.ProcessPoolExecutor(max_workers=64) as executor:
    s30_bands = ['B01', 'B02', 'B03', 'B04', 'B05','B06','B07','B08','B8A','B09','B10', 'B11', 'B12']
    clf = joblib.load('lake_rf_new.pkl')
    results = list(executor.map(process_clip_path, s30_clip_paths, s30_merge_paths, 
      [clf]*len(s30_clip_paths), [res_idx]*len(s30_clip_paths), [im_name]*len(s30_clip_paths),
      ['S30']*len(s30_clip_paths), [s30_bands]*len(s30_clip_paths)))
    ...
    results = list(executor.map(process_clip_path, l30_clip_paths, l30_merge_paths, 
      [clf]*len(l30_clip_paths), [res_idx]*len(l30_clip_paths), [im_name]*len(l30_clip_paths),
      ['L30']*len(l30_clip_paths), [l30_bands]*len(l30_clip_paths)))

    # Combine the results into the main DataFrame
    for df_ in results:
      df = pd.concat([df, df_], ignore_index=True)

    df.to_csv(os.path.join('results',str(res_idx)+'.csv'), index=False)
